...
def index(request):
    if request.method == "POST":
        form = request.POST
        latest_question_list = Question.objects.filter(topic=form.get("select", "")).order_by('-pub_date')
    else:
        latest_question_list = Question.objects.order_by('-pub_date')
    template = loader.get_template('polls/index.html')
    topic_list = list(Question.objects.values('topic').distinct())
    context = {
...
def choice_add(request, question_id):
        ...
        if request.method =='POST':
            ...
            if form.is_valid():
                ...
                choice.vote = 0
                choice.save()
                message= 'Se  ha añadido una respuesta.'
                #form.save()
        else: 
            form = ChoiceForm()
        #return render_to_response ('choice_new.html', {'form': form, 'poll_id': poll_id,}, context_instance = RequestContext(request),)
        return render(request, 'polls/choice_new.html', {'title':'Pregunta:'+ question.question_text,'form': form, 'mensaje': message, 'question':question})
